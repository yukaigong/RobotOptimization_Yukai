function [] = integration_constraints(DOF, OPT_PATH, domainName)
%% Function to generate Integration Constraints
%   Author: Ross Hartley
%   Date:   12/11/2016
%
...
% Position level
constraint = q2 - q1 - deltaT/2*(dq2 + dq1);
vars = [T;q1;dq1;q2;dq2].';
extra = N.';
J_constraint = jacobian(constraint,vars);
symbolicToFunction(['f_intTrapPos_',domainName], {vars,extra}, {constraint}, OPT_PATH)
symbolicToFunction(['J_intTrapPos_',domainName], {vars,extra}, {J_constraint}, OPT_PATH)

% Velocity level
constraint = dq2 - dq1 - deltaT/2*(ddq2 + ddq1);
vars = [T;dq1;ddq1;dq2;ddq2].';
extra = N.';
J_constraint = jacobian(constraint,vars);
symbolicToFunction(['f_intTrapVel_',domainName], {vars,extra}, {constraint}, OPT_PATH)
symbolicToFunction(['J_intTrapVel_',domainName], {vars,extra}, {J_constraint}, OPT_PATH)
