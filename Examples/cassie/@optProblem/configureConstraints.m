    ...
    %%%%%%%%%%%%%%%% Forward Periodic Gait %%%%%%%%%%%%%%%%
    if periodic == 1   
        % Periodicity
        selected = ones(1,DOF); selected(1:3) = 0;
        deps_1 = domains{2}.optVarIndices.qend(end,:);
        deps_2 = domains{1}.optVarIndices.q(1,:);
        domains{2} = addConstraint(domains{2},'Inter-Domain-Nonlinear',...
            'qContSelected',DOF,1,{deps_1,deps_2},-5e-4,5e-4,selected);
        selected = ones(1,DOF);
        deps_1 = domains{2}.optVarIndices.dqend(end,:);
        deps_2 = domains{1}.optVarIndices.dq(1,:);
        domains{2} = addConstraint(domains{2},'Inter-Domain-Nonlinear',...
            'qContSelected',DOF,1,{deps_1,deps_2},-5e-4,5e-4,selected);
    end
    
    if forward_periodic == 1
        
        % Step Time Continuity
        deps_1 = domains{1}.optVarIndices.t(end,:);
        deps_2 = domains{2}.optVarIndices.t(1,:);
        domains{1} = addConstraint(domains{1},'Inter-Domain-Nonlinear',...
            'timeCont',1,domains{1}.nNode,...
            {deps_1,deps_2},-5e-4,5e-4);
        % Bezier Symmetry
        selected = ones(1,DOA);
        deps_1 = domains{1}.optVarIndices.a(end,:);
        deps_2 = domains{2}.optVarIndices.a(1,:);
        domains{1} = addConstraint(domains{1},'Inter-Domain-Nonlinear',...
            'aSymmetrySelected',DOA*(M+1),domains{1}.nNode,...
    ...
    % Initialize state
    deps_1 = domains{1}.optVarIndices.qend(end,:);
    deps_2 = domains{2}.optVarIndices.q(1,:);
    domains{2} = addConstraint(domains{2},'Inter-Domain-Nonlinear',...
        'qCont',DOF,1,{deps_1,deps_2},-5e-4,5e-4);
    deps_1 = domains{1}.optVarIndices.dqend(end,:);
    deps_2 = domains{2}.optVarIndices.dq(1,:);
    domains{2} = addConstraint(domains{2},'Inter-Domain-Nonlinear',...
        'qCont',DOF,1,{deps_1,deps_2},-5e-4,5e-4);
